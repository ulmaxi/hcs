...
describe('RevokeMedicalClaimService', () => {
  ...
  beforeEach(async () => {
    ...
    svc = module.get<RevokeMedicalCarePlanService>(
      RevokeMedicalCarePlanService,
    );
  });
  ...
  describe('deleteAdmission', () => {
    ...
    it('should return the deleted admission', async () => {
      const admission = await module
        .get<AdmissionService>(AdmissionService)
  ...
  describe('deleteLabtests', () => {
    ...
    it('should return the deleted labtests', async () => {
      const saved = await Promise.all(
        testing.labTestFactory
          .buildList(5)
          .map(
            async t =>
              await module
                .get<LabTestService>(LabTestService)
                .repository.save(t),
          ),
      );
      expect(await svc.deleteLabtests(saved.map(s => s.id))).toStrictEqual(
        saved,
      );
    });
  });
  describe('deletePrescriptions', () => {
    ...
    it('should return the deleted prescriptions', async () => {
      const saved = await Promise.all(
        testing.prescriptionFactory
          .buildList(5)
          .map(
            async t =>
              await module
                .get<PrescriptionService>(PrescriptionService)
                .repository.save(t),
          ),
      );
      expect(await svc.deletePrescriptions(saved.map(s => s.id))).toStrictEqual(
        saved,
      );
    });
  });
  ...
  describe('deleteConsultation', () => {
    ...
    it('should return the deleted review', async () => {
      ...
      module
        .get<ConsultationService>(ConsultationService)
        .repository.save(consultation);
      expect(await svc.deleteConsultation(consultation.id)).toStrictEqual(
        consultation,
      );
    });
  });
