...
export class RevokeMedicalCarePlanService {
    ...
    async deleteLabtests(ids?: string[]): RevokedResult<LabTest[]> {
        if (ids) {
            const deleted = await this.labTests.repository.remove(ids.map(id => ({ id })) as any);
            return deleted;
        }
    }
    /**
    ...
    async deletePrescriptions(ids?: string[]): RevokedResult<Prescription[]> {
        if (ids) {
            const deleted = await this.prescription.repository.remove(ids.map(id => ({ id })) as any);
            return deleted;
        }
    }
    ...
    async deleteConsultation(id?: string): RevokedResult<Consultation> {
        if (id) {
            const [deleted] = await this.consultation.repository.remove({ id } as any);
            return deleted;
        }
    }
