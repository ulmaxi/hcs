...
describe('StaffManagmentService', () => {
  ...
  describe('register', () => {
    ...
    it('should throw error that the Staff has already Registered', async () => {
      ...
      jest
        .spyOn(personel, 'retriveTrackIdWithPhoneNo')
    ...
    it('should register the staff the staff successfully', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce('random-tracking-id');
      jest
        .spyOn(staff.repository, 'save')
        .mockResolvedValue({ ...mockStaff, ...details });
      expect(await svc.register('institution', staffDetails)).toStrictEqual({
        ...mockStaff,
        ...details,
      });
    });
  ...
  describe('revoke', () => {
    ...
    it('should throw noStaffWithPhoneNoError', async () => {
      ...
      try {
        ...
        expect(error.message.message).toMatch(
          noStaffWithPhoneNoError(staffPhoneNo).message.message,
        );
      }
    });
    ...
    it('should revoke the staff successfully', async () => {
      ...
      jest.spyOn(staff, 'findOne').mockResolvedValue(mockStaff);
      expect(
        await svc.revoke('insititution', staffDetails.staffPhoneNo),
...
describe('Error factories', () => {
  describe('staffAlreadyRegisteredError', () => {
    it('should contain the phoneNo sent', () => {
      ...
      expect(staffAlreadyRegisteredError(phone).message.message).toContain(
        phone,
      );
    });
  });
