...
export class StaffManagmentService {
  ...
  async register(
    institution: string,
    { staffPhoneNo, ...details }: RegisterStaffDetails,
  ) {
    const trackID = await this.personnel.retriveTrackIdWithPhoneNo(
      staffPhoneNo,
    );
    let staff = await this.staff.findOne({ institution, trackID } as Partial<
      Staff
    >);
    if (staff && !staff.revoked) {
      throw staffAlreadyRegisteredError(staffPhoneNo);
    }
    staff = await this.staff.repository.save({
      ...details,
      institution,
      trackID,
      revoked: false,
 ...
 */
export const staffAlreadyRegisteredError = (staffPhoneNo: string) =>
 ...
 */
export const staffNeverRegisteredError = (staffPhoneNo: string) =>
  new UnprocessableEntityException(
