...
describe('PersonnelService', () => {
    ...
    module = await Test.createTestingModule({
      providers: [
        PersonnelService,
        {
          ...
          useValue: serviceFactoryMock({ factory: staffFactory, preload: 5 }),
        },
        {
  ...
  describe('retriveStaffID', () => {
    ...
    it('should throw noStaffWithPhoneNoError', async () => {
      try {
        jest
          .spyOn(svc, 'retriveTrackIdWithPhoneNo')
          .mockResolvedValueOnce('track-id');
    ...
    it('should return the staff id', async () => {
      ...
      expect(await svc.retriveStaffID(institutions, staffPhoneNo)).toMatch(
        staff.id,
      );
    });
  });
  ...
  describe('retriveTrackIdWithPhoneNo', () => {
    it('should throw formated noPersonnelError', async () => {
      ...
      try {
        ...
        expect(error.toString()).toStrictEqual(
          noPersonnelError(phoneNo).toString(),
        );
      }
    });
    ...
    it('should return the trackId of the personnel', async () => {
      ...
      jest
        .spyOn(microSvc, 'send')
        .mockImplementationOnce(() => of({ trackId }));
