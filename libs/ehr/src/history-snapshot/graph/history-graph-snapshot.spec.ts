...
describe('HistoryGraphSnapshot', () => {
  ...
  it('should create a graph which contains a trackingId', () => {
    ...
    const storedConsultations = db.consultantions.filter(
      c => c.trackId === trackId,
    );
    expect(historyGraph.SnapshotedStore.get(trackId).length).toEqual(
      storedConsultations.length,
    );
  });
});
