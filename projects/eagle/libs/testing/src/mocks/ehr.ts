import { Admission, Consultation, Institution, LabTest, Prescription, Review, Staff } from '@eagle/ehr';
...
export function ehr_data_preload(no: number) {
  const consultantions = flatten(consultationFactory.buildList(no)
    .map((c) => [c, ...consultationFactory.buildList(no * 2, { trackId: c.trackId })]));
  return {
    consultantions,
...
function consultationFields(consultantions: Consultation[]) {
  const admissions = consultantions
    .map(c => admissionFactory.build({ consulationTrackId: c.id }));
  const labtests = flatten(
    consultantions.map(c => c.labtests.map(l => labTestFactory.build({ id: l }))),
  );
  const prescriptions = flatten(
    consultantions.map(
      c => c.prescriptions.map(p => prescriptionFactory.build({ id: p }))));
  return { admissions, labtests, prescriptions };
}

function biodatas(consultantions: Consultation[]) {
  const institutions = consultantions
    .map(c => institutionFactory.build({ id: c.institutionId }));
  const staffs = consultantions
    .map(c => staffFactory.build({ id: c.consultantId }));
  const consultants = staffs
    .map(c => personalBiodataFactory.build({ id: c.trackID }));
