...
describe('PublicDataService', () => {
    ...
    beforeEach(async () => {
        module = await Test.createTestingModule({
            providers: [
                PublicDataService,
                { provide: EmergencyService, useValue: { repository: { save: jest.fn } } },
                {
    ...
    describe('institutions', () => {
        it('should throw error if classification is missing', async () => {
            try {
                ...
                expect((error as Error).message).toEqual(InstitutionClassificationError.message);
            }
        });

        it('should send the request through the client', async () => {
            const classification = 'hospital';
            const hospitals = institutionFactory.buildList(10)
                .filter((i) => i.classification === classification);
            const proxySpy = jest.spyOn(clientSvc, 'send')
    ...
    describe('alertEmergency', () => {
        it('should save and alert the institutions', async () => {
            ...
            const emergencyResponse: EmergencyResponse = {
                emergency,
                institution: omit((institution as any), ['id', 'trackId', 'zipcode', 'country']),
            };
            const saveSpy = spyOn(emergencySvc.repository, 'save')
                .and.returnValue(emergency);
            const insitutionSpy = spyOn(svc, 'institutionDetailsFromName')
                .and.returnValue(of(institution));
            const dispatchSpy = spyOn(svc, 'dispatchEmergency')
                .and.callFake(jest.fn());
